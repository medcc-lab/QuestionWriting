...
if curl -s http://localhost:3000/api/users/profile > /dev/null; then
    echo "Backend server is running (http://localhost:3000/api)"
else
...
if curl -s http://localhost:80 > /dev/null; then
    echo "Frontend server is running (http://localhost)"
else
